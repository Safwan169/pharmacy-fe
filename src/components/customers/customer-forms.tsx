  ...
  return (
    <form action={action} className="space-y-3" noValidate>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Field label="Phone" htmlFor={`phone-${k}`} hint="Used to find them at the counter.">
          <Input id={`phone-${k}`} name="phone" inputMode="tel" defaultValue={customer?.phone ?? ""} maxLength={20} />
        </Field>
        <Field label="Address" htmlFor={`address-${k}`}>
          <Input id={`address-${k}`} name="address" defaultValue={customer?.address ?? ""} maxLength={255} />
        </Field>
      </div>
      <div className="flex gap-2">
        <Button type="submit" disabled={pending}>{pending ? "Saving…" : customer ? "Save" : "Add customer"}</Button>
        {onDone && <Button type="button" variant="ghost" onClick={onDone}>Close</Button>}
      </div>
    </form>
  );
}
...
export function ReceivePayment({ customerId, dueBalance, compact = false }: { customerId: number; dueBalance: number; compact?: boolean }) {
  ...
  if (state.status === "success" && state.payment) {
    return (
      <div className="rounded-lg border border-success/30 bg-success/5 p-3 text-sm">
        ...
        <p className="mt-1 text-xs text-muted">Receipt {state.payment.receiptNumber}</p>
        <a
        ...
        >
          <Printer className="h-3.5 w-3.5" aria-hidden /> Print receipt
        </a>
      </div>
    );
  }
  ...
  if (!open) {
    return (
      <Button type="button" size="sm" variant="secondary" onClick={() => setOpen(true)}>
        <Banknote className="h-3.5 w-3.5" aria-hidden /> Receive payment
      </Button>
    );
  }
  ...
  return (
    <form action={action} className="space-y-2 rounded-lg border border-border bg-background p-3 text-left" noValidate>
      ...
      {state.status === "error" && state.message && <Alert tone="error">{state.message}</Alert>}
      <p className="text-xs text-muted">They owe {formatCurrency(dueBalance)}.</p>
      <div className="grid grid-cols-2 gap-2">
        <Input name="amount" inputMode="decimal" placeholder="Amount" defaultValue={dueBalance > 0 ? String(dueBalance) : ""} aria-label="Amount received" className="tabular-nums" />
        <Select name="method" value={method} onChange={(e) => setMethod(e.target.value as "cash" | "bkash")} aria-label="Paid by">
          <option value="cash">Cash</option>
          <option value="bkash">bKash</option>
        </Select>
      </div>
      {method === "bkash" && <Input name="bkash_trx_id" placeholder="bKash TrxID (optional)" maxLength={30} className="font-mono uppercase" aria-label="bKash TrxID" />}
      <Input name="note" placeholder="Note (optional)" maxLength={255} aria-label="Note" />
      <div className="flex gap-2">
        <Button type="submit" size="sm" disabled={pending}>{pending ? "Saving…" : "Record payment"}</Button>
        {compact && <Button type="button" size="sm" variant="ghost" onClick={() => setOpen(false)}>Cancel</Button>}
      </div>
    </form>
  );
}
