  ...
  return (
    <>
      ...
      >
        ...
        <span className="hidden sm:inline">{t("password.button")}</span>
      </button>

      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <button type="button" aria-label={t("password.close")} onClick={() => setOpen(false)} className="absolute inset-0 bg-foreground/30" />
          <div className="relative w-full max-w-sm rounded-xl border border-border bg-surface p-5 shadow-xl">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setOpen(false)} aria-label={t("password.close")} className="rounded-md p-1 text-muted hover:text-foreground">
            ...
            <form action={action} className="mt-4 space-y-3" noValidate>
              ...
              <Field label={t("password.current")} htmlFor="current_password">
                <Input id="current_password" name="current_password" type="password" autoComplete="current-password" />
              </Field>
              <Field label={t("password.new")} htmlFor="new_password" hint={t("password.newHint")}>
                <Input id="new_password" name="new_password" type="password" autoComplete="new-password" />
              </Field>
              <Field label={t("password.confirm")} htmlFor="confirm_password">
                <Input id="confirm_password" name="confirm_password" type="password" autoComplete="new-password" />
              </Field>
              <Button type="submit" disabled={pending} className="w-full">
                {pending ? t("common.saving") : t("password.submit")}
              </Button>
            </form>
          </div>
        </div>
